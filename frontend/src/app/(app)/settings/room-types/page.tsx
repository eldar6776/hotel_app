      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <input required placeholder="Kod" value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-text" />
          </div>
          <div className="grid grid-cols-3 gap-3">
            <input type="number" min="1" placeholder="Osnovni kap." value={form.baseCapacity} onChange={e => setForm({ ...form, baseCapacity: Number(e.target.value) })} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-text" />
            <input type="number" min="1" placeholder="Max kap." value={form.maxCapacity} onChange={e => setForm({ ...form, maxCapacity: Number(e.target.value) })} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-text" />
            <input type="number" step="0.01" min="0" placeholder="Cijena" value={form.defaultPrice} onChange={e => setForm({ ...form, defaultPrice: Number(e.target.value) })} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-text" />
          </div>
          <input placeholder="Opis (opciono)" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-text" />
