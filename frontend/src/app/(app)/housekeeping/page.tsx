      ...
      {isLoading ? (
        ...
        dirtyRooms.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {dirtyRooms.map((room) => (
              <div key={room.id} className="rounded-xl border border-orange-200 dark:border-orange-800 bg-surface p-4 shadow-sm">
                ...
                {inspectingRoom === room.id ? (
                  <div className="space-y-2 mt-2">
                    <input
                      ...
                      className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-xs text-text"
                    />
        ...
        <div className="space-y-3">
          ...
          {showWoForm && (
            <form onSubmit={handleCreateWorkOrder} className="rounded-xl border border-border bg-surface p-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <select
                  value={woForm.roomId}
                  onChange={(e) => setWoForm({ ...woForm, roomId: e.target.value })}
                  className="rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm text-text"
                  required
                >
          ...
          {workOrders.length === 0 ? (
            ...
            <div className="space-y-2">
              {workOrders.map((wo) => (
                <div key={wo.id} className="rounded-xl border border-border bg-surface p-4 shadow-sm">
