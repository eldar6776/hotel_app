...
export function GuestDetail({ guest, onClose }: { guest: GuestDto; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="w-full max-w-3xl rounded-xl bg-surface p-6 shadow-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          ...
          <div className="space-y-2">
            {profile.stayHistory.length === 0 ? (
              ...
              profile.stayHistory.map(s => (
                <div key={s.bookingId} className="rounded-lg border border-border bg-surface-secondary p-3 flex justify-between items-center">
                  <div>
          ...
          <div className="space-y-2">
            {profile.bookingHistory.length === 0 ? (
              ...
              profile.bookingHistory.map(b => (
                <div key={b.bookingId} className="rounded-lg border border-border bg-surface-secondary p-3 flex justify-between items-center">
                  <div>
